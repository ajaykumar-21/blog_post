...
test("post validation accepts a normal post and rejects excess tags", () => {
  assert.equal(
    validatePost({ title: "A title", content: "A body", tags: ["nextjs"] }),
    null,
  );
  assert.equal(
    validatePost({
    ...
    "Use up to five non-empty tags.",
  );
});

test("credential validation requires an email and secure-length password", () => {
  assert.equal(
    validateCredentials({ email: "user@example.com", password: "password123" }),
    null,
  );
  assert.equal(
    validateCredentials({ email: "not-an-email", password: "password123" }),
