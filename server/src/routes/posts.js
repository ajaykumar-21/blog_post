...
async function assignTags(client, postId, tagNames = []) {
  const normalized = [
    ...new Set(tagNames.map((tag) => tag.trim().toLowerCase())),
  ];
...
router.get("/", async (req, res, next) => {
  ...
  if (req.query.q?.trim()) {
    ...
    clauses.push(
      `to_tsvector('english', coalesce(p.title, '') || ' ' || coalesce(p.content, '')) @@ websearch_to_tsquery('english', ${query})`,
    );
  }
  if (req.query.tag?.trim()) {
    const tag = add(slugify(req.query.tag));
    clauses.push(
      `EXISTS (SELECT 1 FROM post_tags ptx JOIN tags tx ON tx.id = ptx.tag_id WHERE ptx.post_id = p.id AND tx.slug = ${tag})`,
    );
  }
  try {
    const where = clauses.join(" AND ");
    const total = await pool.query(
      `SELECT COUNT(*)::int AS count FROM posts p WHERE ${where}`,
      values,
    );
    const queryValues = [...values, limit, (page - 1) * limit];
    const { rows } = await pool.query(
      `SELECT ${postColumns} FROM posts p LEFT JOIN users u ON u.id = p.author_id
      WHERE ${where} ORDER BY p.created_at ${sort}, p.id ${sort} LIMIT $${queryValues.length - 1} OFFSET $${queryValues.length}`,
      queryValues,
    );
...
router.get("/mine", requireAuth, async (req, res, next) => {
  try {
    const { rows } = await pool.query(
      `SELECT ${postColumns} FROM posts p LEFT JOIN users u ON u.id = p.author_id WHERE p.author_id = $1 ORDER BY p.updated_at DESC`,
      [req.user.id],
    );
    res.json(rows);
  } catch (error) {
...
router.get("/:idOrSlug", async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT ${postColumns} FROM posts p LEFT JOIN users u ON u.id = p.author_id WHERE ${identifier} AND p.status = 'published'`,
      [value],
    );
    if (!rows[0]) return res.status(404).json({ error: "Post not found." });
    const comments = await pool.query(
      `SELECT c.id, c.post_id, c.parent_id, c.content, c.created_at, c.user_id,
...
router.post("/", requireAuth, async (req, res, next) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      `INSERT INTO posts (title, content, author, author_id, slug, status)
...
router.put("/:id", requireAuth, async (req, res, next) => {
  ...
  try {
    ...
    const { rows, rowCount } = await client.query(
      `UPDATE posts SET title=$1, content=$2, status=$3, updated_at=CURRENT_TIMESTAMP
  ...
  try {
    ...
    if (parentId) {
      ...
      if (parent.rows[0].parent_id)
        return res
          .status(400)
          .json({ error: "Replies can be one level deep." });
    }
    const { rows } = await pool.query(
      `INSERT INTO comments (post_id, parent_id, user_id, name, content)
      VALUES ($1,$2,$3,$4,$5) RETURNING id,post_id,parent_id,user_id,name AS author,content,created_at`,
      [
        req.params.id,
        parentId,
        req.user.id,
...
router.delete("/comments/:id", requireAuth, async (req, res, next) => {
  try {
    ...
    if (!result.rowCount)
      return res
        .status(404)
        .json({ error: "Comment not found or not owned by you." });
    res.status(204).end();
  } catch (error) {
    next(error);
  }
